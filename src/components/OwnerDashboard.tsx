...
import { Booking, HardwareItem, CrewLead, ChatMessage, SystemAuditLog, Role } from "../types";
import { 
  MessageSquare, 
  ShieldCheck, 
  Clipboard, 
...
interface OwnerDashboardProps {
  ...
  onAssignCrew: (bookingId: string, crewId: string) => void;
...
export default function OwnerDashboard({
  ...
  // "dashboard" | "spreadsheet" | "calendar" | "crm" | "accounts" | "performance"
  const [activeMenu, setActiveMenu] = useState<"dashboard" | "spreadsheet" | "calendar" | "crm" | "accounts" | "performance">("dashboard");
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-neutral-900/60 border border-white/10 rounded-[2rem] p-6 backdrop-blur-md">
                <h3 className="text-sm font-bold font-display uppercase tracking-wider text-white mb-4">
                  ⚡ Hardware & Printer Media Thresholds
                </h3>
                <div className="space-y-2.5">
                  {hardwareList.map((hw) => {
                    const isLow = hw.stockCount <= hw.threshold;
                    return (
                      <div key={hw.id} className="flex items-center justify-between p-3 bg-black/30 border border-white/5 rounded-xl text-xs">
                        <div>
                          <span className="text-white font-semibold">{hw.name}</span>
                          <span className="text-[9px] text-gray-500 block uppercase font-mono mt-0.5">{hw.category}</span>
                        </div>
                        <div className="text-right">
                          <span className={`font-mono font-bold ${isLow ? "text-amber-500" : "text-gray-400"}`}>
                            Stock: {hw.stockCount} / {hw.threshold}
                          </span>
                          {isLow && (
                            <span className="text-[8px] text-red-400 font-bold uppercase block mt-0.5">⚠️ Reorder Print Sheets</span>
                          )}
                        </div>
                      </div>
                    );
        ...
        <div className="bg-neutral-900/60 border border-white/10 rounded-[2rem] p-6 backdrop-blur-md space-y-6">
          ...
          <div className="overflow-x-auto rounded-xl border border-white/10">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-black/40 border-b border-white/10 text-gray-400 font-mono">
                  <th className="p-4">Client Name</th>
                  <th className="p-4">Package Order</th>
                  <th className="p-4">Location Venue</th>
                  <th className="p-4">Person In-charge</th>
                  <th className="p-4 text-center">Duration</th>
                  <th className="p-4 text-right">Package Price</th>
                  <th className="p-4 text-center">Approval Progress</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {filteredBookings.length === 0 ? (
                  ...
                  filteredBookings.map((b) => {
                    const matchedCrew = crewLeads.find((c) => c.id === b.crewLeadId);
                    const hours = PACKAGES.find((p) => p.name.toLowerCase() === b.packageName.toLowerCase())?.durationHrs || 3;
                    const isApproved = b.status === "BOOKED";

                    return (
                      <tr key={b.id} className="hover:bg-white/5 transition-all">
                        <td className="p-4 font-bold text-white">{b.clientName}</td>
                        <td className="p-4 text-gray-300">
                          <span className="bg-neutral-800 px-2 py-1 rounded text-[10px] font-semibold text-[#a1c398]">
                            {b.packageName}
                          </span>
                        </td>
                        <td className="p-4 text-gray-400 max-w-[180px] truncate" title={b.locationAddress}>
                          {b.locationAddress || "Selangor Area"}
                        </td>
                        <td className="p-4">
                          <select
                            value={b.crewLeadId || ""}
                            onChange={(e) => onAssignCrew(b.id, e.target.value)}
                            className="bg-black text-gray-300 border border-white/10 rounded px-2.5 py-1 text-xs outline-none focus:border-[#799351]"
                          >
                        ...
                        <td className="p-4 text-center font-mono text-gray-300">{hours} Hrs</td>
                        <td className="p-4 text-right font-mono font-bold text-[#a1c398]">RM {b.totalPrice}</td>
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-1.5">
                            {isApproved ? (
                              <span className="flex items-center gap-1 text-[10px] bg-emerald-950 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-full font-mono">
                              ...
                              <button
                                onClick={() => handleApprove(b.id)}
                                className="flex items-center gap-1 text-[10px] bg-amber-950/50 hover:bg-amber-950 text-amber-400 border border-amber-500/20 px-2.5 py-1 rounded-full font-mono transition-colors"
                              >
